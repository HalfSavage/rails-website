class Post < ActiveRecord::Base
  ...
  # Hooks
  before_validation :create_slug, if: :post_is_parent_of_discussion?
  ...
  validate :member_has_permission_to_post_in_forum

  # Validations that apply only if this post is the parent of a new discussion (a.k.a. "thread")
  validates :subject, length: {minimum: 5, maximum: 200}, if: :post_is_parent_of_discussion?
  validates_presence_of :forums_posts, if: :post_is_parent_of_discussion?, message: "...discussion must belong to some forums"
  validates_presence_of :subject, if: :post_is_parent_of_discussion?
  validates_presence_of :slug, if: :post_is_parent_of_discussion?
  ...
  validates_absence_of :forums_posts, unless: :post_is_parent_of_discussion?
  validates_absence_of :subject, unless: :post_is_parent_of_discussion?
  validates_absence_of :slug, unless: :post_is_parent_of_discussion?
  ...
  def create_slug 
    self.slug = subject.parameterize if (self.subject.present? && self.slug.blank?)
  ...
  def post_is_parent_of_discussion?
    parent.nil?
  end 

  def post_is_not_parent_of_discussion?
    !post_is_parent_of_discussion?
  end 
  ...
  def only_in_moderator_forums?
    return !in_at_least_one_public_forum?
  end 

  def member_has_permission_to_post_in_forum
    # is member banned?
    ...
    if self.post_is_parent_of_discussion? 
      the_forums = forums 
    else 
    ...
    the_forums.each do |f| 
      has_permission = true if !f.paid_member_only? || member.paid?
    end 
    errors[:base] << "Only paid members may post in this forum" if !has_permission 

    # if forum is mod-only, is member mod?
    # note: if discussion exists in multiple forums, user only needs permission for one of them!
    # todo: should refactor to use only_in_moderator_forum? above
    has_permission = false 
    the_forums.each do |f|
      has_permission = true if !f.moderator_only || member.moderator?
    end 
    errors[:base] << "Only moderators may post in this forum" if !has_permission 
  end 
