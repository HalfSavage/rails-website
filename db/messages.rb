...
module HalfSavageMessagesModule
end 

class HalfSavageMessages 
  extend HalfSavageMessagesModule
...
protected
  ...
  @@related_person = %w{sister brother teacher doctor mom dad best\ friend neighbor classmate totally\ gay\ friend co-worker creepy\ neighbor pregnant\ cousin best\ friend boyfriend bride bridegroom brother bud buddy manager bro homie lover neighbor newborn sexually\ confused\ MMA\ opponent fuck-buddy rival creepy\ roommate anal\ slave}
...
=begin
puts "---Post replies---"
HalfSavageMessages.test_post_reply_sentence_templates
puts "\n---Post 2nd sentences--"
HalfSavageMessages.test_post_reply_second_sentence_templates
puts "\n---Private message openers--"
HalfSavageMessages.test_private_message_sentence_templates
puts "\n---Private message 2nd sentences--"
HalfSavageMessages.test_private_message_second_sentence_templates
puts "\n---Regular filler sentences---"
HalfSavageMessages.test_sentence_templates
(1..6).each { |x| 
  puts "\n---Private message body (~#{x} sentences)---"
  puts HalfSavageMessages.get_private_message_body(x..x)
}
(1..6).each { |x| 
  puts "\n---Post reply message body (~#{x} sentences)---"
  puts HalfSavageMessages.get_private_message_post_reply_body(x..x)
}
=end