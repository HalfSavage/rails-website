...
class HalfSavageMessages 
  def initialize 
    ...
    @body_parts = ['asshole', 'yiffhole', 'boner', 'fuckhole', 'taint', 'tits', 'dick',  'vagina', 'butthole', 'abdomen', 'Adam\'s apple', 'forehead', 'anus', 'butt dimples', 'distended belly', 'belly button', 'gallbladder', 'brain', 'left titty', 'buttocks', 'fucking face', 'cervix', 'fallopian tubes', 'stretch marks', 'bush', 'liver', 'circulatory system', 'dingus', 'sexual areas', 'collar bone', 'diaphragm', 'digestive system', 'ear', 'ear lobe', 'elbow', 'endocrine system', 'esophagus', 'eye', 'eyelashes', 'eyelid', 'face', 'fallopian tubes', 'feet', 'femur', 'fibula',  'foot', 'forehead', 'gallbladder', 'groin', 'gums', 'hair', 'hand', 'head', 'aorta', 'heel', 'hip', 'humerus', 'immune system', 'instep', 'intestines', 'jaw', 'kidney', 'knee', 'larynx', 'leg', 'nasty mouth', 'lip', 'liver', 'lobe', 'lumbar vertebrae', 'lungs', 'beautiful mouth', 'face', 'everything', 'sensitive areas', 'tonsils', 'mouth', 'muscle', 'belly button', 'neck', 'nerves', 'nipple', 'nose', 'nostril', 'organs', 'ovary', 'palm', 'pancreas', 'patella', 'pelvis', 'nuts', 'pharynx', 'leaky wound', 'eyes','rectum', 'red blood cells', 'respiratory system', 'ribs', 'sacrum', 'scalp', 'scapula', 'senses', 'shin', 'shoulder', 'shoulder blade', 'skeleton',
     'skin', 'skull', 'spleen', 'stomach', 'forbidden anus', 'silky pleasure entrance', 'testes', 'creamy thighs', 'bottomless hole', 'throat', 'useless stumps', 'toenail', 'tongue', 'tonsils', 'teeth', 'torso', 'butt cheeks', 'urethra', 'ding-dong', 'uterus', 'uvula', 'extra nipple', 'vulva']
    ...
    @housing_adjectives = %w{fixer-upper nasty opulent charming elegant filthy condemned infested cleanest}
    ...
    @emotions = ['remorse', 'melancholy', 'emotions', 'regrets', 'joyousness', 'happiness', 'panic', 'erotic mayhem', 'pants-shitting rage', 'cold, methodical vengeance']
    ...
    @sexual_noun = ['prostate massage','marathon hatefucking','passionate kiss that will leave you wanting more','confusing finger in your butt','sexual awakening','reasonably energetic lay','nipple savaging','awkward fingerbang','orgasm','blowjob','handjob','fingerbang','nipplejob','load of man-batter','shoebox full of pubes','sexual ass-kicking','haymaker to the taint']
    ...
    @private_message_sentence_templates = [
      "Get ready for a good #{@verbs_hitting.sample} at #{@events.sample}! Afterwards, surely we'll #{@activities.sample}.",
      "I #{@desires.sample} some #{@food_types.sample}!",
      "#{['Except you, nobody ever talks about the','Only you understand the','You taught me so much about the'].sample} #{@emotions.sample} of #{@diseases.sample}. Thanks to you, I'm #{@personal_enhancements.sample}.",
      "Thanks for the crazy #{@body_parts.sample}-fucking! #{['I could barely concentrate on','Can''t believe you still had time for','Sure beats'].sample} #{@verbs_sexual.sample} #{['your','my'].sample} #{@related_person.sample}!",
      "I've got to #{@gush_or_gripe.sample} about your #{@body_parts.sample} and mastery of #{@academic_topics.sample}!",
      "#{['If you','Unless you'].sample} #{@reactions.sample} I'll give you a #{@verbs_sexual.sample} that will make you say, '#{@exclamations.sample}'",
      "#{['We could have a good time with my','Let''s have a crazy time with your'].sample} #{(@game_consoles + @food_types).sample} in my #{@housing.sample} #{['if you','unless you'].sample} #{@reactions.sample}.",
      "#{['I see you''re from','You live in','What are you doing in'].sample} #{@places_proper.sample}? I thought that #{@housing_events.sample}?",
      "#{@adjectives_sexual.sample(3).join('... ')}... that's how they describe my #{@body_parts.sample}.",  
      "#{@exclamations.sample} Were you the one #{@verbs_hitting.sample} my #{@related_person.sample} and doing #{@vices_plural.sample}? You're a #{@adjectives_sexual.sample}.",  
      "My #{@rooms.sample}... your #{(@diseases + @game_consoles + @academic_topics).sample}... my #{@adjectives.sample} #{@furnishings.sample}... sound good? Let's #{@activities.sample}!",
      ...
      "#{['You''ve been stalking me for years and now I''m stalking you','What a coincidence!','I think I know you!', 'We meet again!'].sample} I'm the person with the #{(@game_consoles + @diseases + @animals).sample} that you #{@verbs_past.sample} at #{@events.sample}#{%w{! . \ ...right?}.sample}",
      ...
      "#{['There''s no way I''d be caught dead at', 'I''d rather die than check out your', 'Thanks for inviting me to','I got the invitation to','I\'d love to join you at'].sample} your #{['party','dad\'s funeral','circumcision','sex club','birthday party','sad little gathering','frat house','sex mattress in the woods'].sample}, but I'll be busy #{@personal_enhancements.sample} tonight.",
    ...
    @post_reply_second_sentence_templates = [
      ...
      "Your #{['posts','sporadic musings','writings','words'].sample} are a #{['fountain','cesspool','untamed ejeculation','punch to the head','simmering stew'].sample} of #{['knowledge','laughter','inspiration','fucking bullshit','red-hot fucksauce'].sample}#{%w{. !}.sample}",
      "I think one of your forum posts gave me #{@diseases.sample}."
    ]
    ...
    @private_message_closer_templates = [
      "Fuck... fuck.... ahhhhh damn... I just shit my pants. Gotta go.",
      "I'm going to end this message now, because the drugs are wearing off.",
      ...
      "I hope I can #{["buy you a drink","play with your action figures","open my emotions up to you","finally remember your name","forget you ever existed"].sample} someday! Hopefully I'll be #{@verbs_sexual.sample} you at #{@events.sample} later this year.",
      "So yeah, #{["it'll be fun","no pressure, it won't be a date or anything","let's hang","bring your "].sample}. More like #{['the best day of your fucking life', 'a trip to the dentist\'s office, except you\'re having sex with me','your first day in a cult','an AWESOME date', '...wait, actually I have no idea what a date is. I am so lonely'].sample}. #{['Please say yes!','Consider my demands.','We will talk soon!','See ya.'].sample}"
    ]


    @sentence_templates = [
      "#{['Been too long since our last collaboration!','I know your mom''s in a coma, but whatever.','Take your hands off your dick for ten seconds and listen to this--','If you''re not too busy with school, I have an idea.'].sample} Help me create a #{@mediums.sample} about a #{@type_of_person.sample} with a #{(@body_parts + @related_person).sample} that can #{['really please the ladies','destroy almost anything - including the Earth','fuck shit up','cook almost any kind of frozen food','shoot lasers','make some crazy farting noises'].sample}.",
      ...
      "Maybe this is why #{%w{everybody nobody}.sample} likes me.",
      "#{['It''s just like...','I ask myself,'].sample} #{['WHOA','damn','hmmm','tee hee hee','hahaha','holy fuck','... ... ...'].sample}#{['...',''].sample} #{['why are we even here in this physical world?', 'what is the meaning of love, anyway?', 'can we ever truly understand another person\'s heart?', 'I wish I was banging a huge black dude right now, honestly.', 'how much vodka did I pour into my butt, anyway?', 'sometimes I wish I wasn\'t a mutant.', 'this has been a really tough year for all of us at Hogwarts.'].sample}"
    ]
