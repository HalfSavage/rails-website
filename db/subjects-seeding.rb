...
class HalfSavageSubjects
  def initialize
    ...
    @places_proper = ['Jersey City', 'Hoboken', 'Beirut', 'Afghanistan', 'Poughkeepsie', 'New York', 'Iowa', 'my hood', 'Compton', 'Western Canada', 'this lousy town', 'the ghetto', 'Hell', 'outer space', 'France', 'Germany', 'Detroit', 'Philadelphia', 'Atlanta', 'the middle of the Sahara Desert', 'Texas', 'Australia', '', 'Mongolia', 'San Francisco', 'Oakland', 'Planet Telezart', 'Florida']
    @places_outdoor = %w{swamp forest desert wilderness tundra jungle lava\ field}
    @activities = ['chain myself to a bear', 'join a cult','have a quiet nipple moment', 'find love', 'get laid', 'get murdered', 'learn how to love', 'find a unicorn', 'play ice hockey', 'have the living shit kicked out of me', 'achieve inner peace', 'fuck a hole in the wall', 'get diseases', 'be homeless', 'masturbate', 'touch a hobo', 'knit sweaters until my fingers fall off']
    ...
    @game_console_features = ['sexual graphics', 'artificial girlfriend simulations', 'homebrew software', 'microchips and shit', 'controller ports', 'electronic jibber-gabber', 'megaflops', 'pixels', 'sensitive areas', 'graphics chips', 'blast processing', 'optical drive', 'USB-powered rubber vagina']
    ...
    @type_of_person = ['grim-faced hobo', 'incredibly weathered prostitute', 'aggressively sensual martial artist', 'construction worker with a tramp stamp', 'robot with a mysterious past', 'killer robot', 'aging liberal with a lisp', 'creepy lumberjack', 'imp', 'terrified child', 'libertarian asshole', 'confused elderly man', 'chubby nerd', 'Christian fundamentalist', 'meth addict', 'black guy', 'poet', 'biker', 'double amputee', 'Pokemon master', 'femme fatale', 'toddler', 'hobo', 'Mexican drug lord', 'frat guy', 'hipster', 'librarian', 'austistic savant']
    ...
    @subject_templates = [
      ...
      "#{["Usually", "Y'know", "Occasionally", "Often"].sample}, when I #{["rock the mic","bust a move","drink fifty shots of tequila","challenge some fool to a dance-off","start a prison riot"].sample}, #{["I get so funky","things happen so quickly","bodily fluids spray so abundantly","so many people die","I get so turned on","bitches be so breathless"].sample} that #{["I just gotta Instragram it","you can't possibly beat your dick fast enough","everybody asks me to stop","they call the cops","my mom has to take me home","nothing in your life will ever compare"].sample}.",
      "If this #{["hoagie","sandwich","cassrole","burger","Hot Pocket"].sample} was a #{["vagina","sensual body part","erotic zone","ball sack","nipple","turgid boner"].sample}, I'd be #{(@verbs_sexual + @verbs_hitting).sample} it.",
      "My #{@related_person.sample} won't let me #{@activities.sample}.#{["","","","",""," Is murder the answer?"," Would a bribe help?"," Right or wrong?"].sample}",
      "#{["This","Next","This"].sample} #{["morning","afternoon","summer","winter","spring","year","fall"].sample}, I'm going to #{["hitchhike","backpack","fuck my way","hike","drive","travel","make my way"].sample} across #{["Russia","America","Europe","Texas","Australia","Antarctica","the solar system","China"].sample} and #{["invade the fuck out of it","achieve sexual domination", "start some shit","make my own religion","see how many hobos I can kiss","learn martial arts","probably get pregnant","find God","see if I can write a book"].sample}.#{[" While high."," Using my body as currency."," Backwards."," Can you give me money?"," Just like my dad.", " While drunk the entire time.", " I'm probably gonna die, right?", "","",""].sample}",
      "#{["Will you","Can you","Please,"].sample} #{["review","critique","help me write"].sample} my #{["fan fiction","manuscript","new story","drunken scrawlings","story","poem"].sample} about themes of #{["remorse","drinking","anal sex","symbolism","mayhem","sensitive moments"].sample} and #{["allegory","danger","redemption","blowjobs","revenge","ennui","existentialism","filthy women"].sample}?#{["","","",""," I think it's good."," It's for my mom."," It is over 1,000 pages long.", " This is good shit.", " I accidentally wrote it in Klingon."].sample}"
    ]
  end 
